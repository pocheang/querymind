...
- Answers cite facts taken from a tool result with their own markers, `[T1]`, `[T2]`, listed under **Tool sources** after the references. Such sentences are checked against the tool result instead of being hedged as unsupported, and an answer built from tool results alone is checked rather than skipped. A specialist's own regex extractions are never citable.
- Answer validation no longer reads citation markers (`[1]`, `[E1]`, `[T1]`) as numbers the sources must contain, and no longer checks a Chinese answer's wording for "entities" against a source that contains no Chinese text.
