...
def test_ci_quality_gate_runtime_unavailable_fails_by_default_and_can_be_overridden():
    ...
    # --allow-runtime-unavailable only converts exit code 3 (runtime unavailable) to 0
    # Exit code 4 (recall below threshold) is a real failure and should not be suppressed
    if proc_default.returncode == 3:
        assert proc_allow.returncode == 0
    elif proc_default.returncode == 4:
        assert proc_allow.returncode == 4
