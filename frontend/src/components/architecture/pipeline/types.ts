...
export const STEPS: StepConfig[] = [
  ...
  },
  {
    ...
    simStatusZh: "调度指挥官：进行中文语义分词与意图判定，选择最合适的检索路由...",
    simStatusEn: "Router: Segmenting NLP tokens, determining intent, choosing a retrieval route...",
    keyPointsZh: ["Jieba 中文语义分词", "路由决策缓存", "低置信度时推理模型复判", "向量 / 图谱 / 混合 / 联网 / 工具"],
    keyPointsEn: [
      "Jieba Token Preprocessing",
      "Route Decision Cache",
      "Reasoning-model Re-ask on Low Confidence",
      "Vector / Graph / Hybrid / Web / Tool",
    ],
    inputZh: "清洗后的用户自然语言提问",
    inputEn: "Sanitized natural language query",
    outputZh: "执行路由分支 (RAG / ReAct / Graph) + 路由置信度",
    outputEn: "Routing Decision (RAG / ReAct / Graph) + Route Confidence",
    securityZh: "低置信度复判、指定智能体类别时直接路由",
    securityEn: "Low-confidence re-ask; a chosen agent class routes directly",
  },
  {
    id: 3,
    stepKey: "step3",
  ...
  },
  {
    ...
    keyPointsEn: [
      ...
      "NLI Entailment (lexical scorer for Chinese)",
    ...
    securityZh: "NLI 蕴含推断（中文使用词汇评分）、分块 DLP 敏感词脱敏",
    securityEn: "NLI entailment (lexical scorer for Chinese), streaming DLP redaction",
  },
];
