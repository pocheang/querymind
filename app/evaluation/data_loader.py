...
def save_test_queries(queries: list[TestQuery], file_path: str | Path) -> None:
    ...
    data = {"queries": [query.model_dump() for query in queries]}

    with open(file_path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)


def filter_queries_by_category(queries: list[TestQuery], category: str) -> list[TestQuery]:
...
def filter_queries_by_difficulty(queries: list[TestQuery], difficulty: str) -> list[TestQuery]:
