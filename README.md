...
│  ├─────────────────────────────────────────────────────────────────┤ │
│  │  ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐        │ │
│  │  │ Router  │──▶│ Vector  │   │  Graph  │   │   Web   │        │ │
│  │  │  Agent  │   │   RAG   │   │   RAG   │   │ Search  │        │ │
│  │  │ 路由分析 │   │ Agent   │   │ Agent   │   │  Agent  │        │ │
│  │  └─────────┘   │ 向量检索 │   │ 图谱查询│   │ 网络搜索│        │ │
│  │                └────┬────┘   └────┬────┘   └────┬────┘        │ │
│  │                     └──────────────┼─────────────┘             │ │
│  │                                    ▼                            │ │
...
| Agent | 触发条件 | 执行逻辑 | 输出 |
|-------|---------|---------|------|
| **Router** | 每次查询 | 意图分析，选择执行路径 | 路由决策 |
| **Vector RAG** | 知识型查询 | 向量+BM25 混合检索 | 文档片段列表 |
| **Graph RAG** | 关系型查询 | Neo4j 实体关系查询 | 知识图谱结果 |
| **Web Search** | 本地知识不足 | 网络搜索（可配置） | 网络搜索结果 |
| **Synthesis** | 最后阶段 | LLM 生成+安全检查 | 最终答案+引用 |
